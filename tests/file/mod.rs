use actiondb::matcher::pattern::file::File;
...
fn test_given_a_valid_json_pattern_file_when_it_is_deserialized_then_we_can_extract_the_patterns_from_it() {
    ...
    let file = File::open(&file_name).ok().expect("Failed to load JSON serialized Pattern");
